...
//public class WizardJsonGrammarNewXtextProjectCreationPage extends WizardNewXtextProjectCreationPage{
/**
 * Copied from org.eclipse.xtext.xtext.ui.wizard.project.WizardNewXtextProjectCreationPage
 * @author aless
 *
 */
public class WizardJsonGrammarNewXtextProjectCreationPage extends WizardNewProjectCreationPage{
//
//	public WizardJsonGrammarNewXtextProjectCreationPage(String pageName, IStructuredSelection selection) {
//		super(pageName, selection);
//	}
	public static final String JAVA_11_BREE = "JavaSE-11";
	private static final Pattern PATTERN_EXTENSIONS = Pattern.compile("\\w+(?:,\\s*\\w+)*"); //$NON-NLS-1$
	private Text languageNameField;
	
	private String initialLanguageNameField=null; // added by Alessandro
	...
	private String initialExtensionsField = null;
	...
	protected void setDefaults(String projectSuffix) {
		if(this.initialLanguageNameField==null) {
			languageNameField.setText("org.xtext.example." + projectSuffix + ".MyDsl"); //$NON-NLS-1$ //$NON-NLS-2$
		}else {
			languageNameField.setText(initialLanguageNameField);
		}
		if(this.initialExtensionsField==null) {
			extensionsField.setText(projectSuffix);
		}else {
			extensionsField.setText(initialExtensionsField);
		}
		
		breeCombo.select(breeCombo.indexOf(JREContainerProvider.getDefaultBREE()));
		breeCombo.select(breeCombo.indexOf(JAVA_11_BREE));
		validatePage();
	}
	...
	private boolean isInitialProjectFieldValueSet =false;
	
	

	@Override
	public void setInitialProjectName(String name) {
		if(! this.isInitialProjectFieldValueSet) {
			this.isInitialProjectFieldValueSet=true;
			super.setInitialProjectName(name);
		}
	}
	
	
	public String getInitialLanguageNameField() {
		return initialLanguageNameField;
	}

	public void setInitialLanguageNameField(String initialLanguageNameField) {
		this.initialLanguageNameField = initialLanguageNameField;
	}
	
	public String getInitialExtensionsField() {
		return initialExtensionsField;
	}

	public void setInitialExtensionsField(String initialExtensionsField) {
		this.initialExtensionsField = initialExtensionsField;
	}
