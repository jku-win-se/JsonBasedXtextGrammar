...
public class AdvancedJsonGrammarNewProjectPage extends WizardPage{
	...
	public AdvancedJsonGrammarNewProjectPage(String pageName) {
		super(pageName);
...
//		select(sourceLayout, SourceLayout.PLAIN)
		...
		createSDKProject.setSelection(true);
		createSDKProject.setEnabled(false);
		createP2Project.setSelection(true);

		select(preferredBuildSystem, BuildSystem.MAVEN);
		select(createLanguageServer, LanguageServer.NONE);
		select(sourceLayout, SourceLayout.PLAIN);
	}
